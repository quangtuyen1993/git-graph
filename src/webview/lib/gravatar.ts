...
export function md5(input: string): string {
  ...
  function md5blks(s: string) {
    ...
    for (let i = 0; i < 64; i += 4) {
      md5blk[i >> 2] = i < n ? s.charCodeAt(i) + (i + 1 < n ? s.charCodeAt(i + 1) << 8 : 0) + (i + 2 < n ? s.charCodeAt(i + 2) << 16 : 0) + (i + 3 < n ? s.charCodeAt(i + 3) << 24 : 0) : 0;
    }
    return md5blk;
  }
  ...
  for (i = 64; i <= n; i += 64) {
    ...
    for (let j = 0; j < 64; j += 4) {
      const idx = i - 64 + j;
      blk[j >> 2] = str.charCodeAt(idx) + (str.charCodeAt(idx + 1) << 8) + (str.charCodeAt(idx + 2) << 16) + (str.charCodeAt(idx + 3) << 24);
    }
    md5cycle(state, blk);
  }
