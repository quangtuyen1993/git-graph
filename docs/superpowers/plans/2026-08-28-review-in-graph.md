...
| File | Responsibility |
|---|---|
| `src/webview/components/detail/ReviewDetail.svelte` | The detail panel's third mode: one review — target, derived base, progress, body, failure reason |
...
**Files:** create `src/webview/components/detail/ReviewDetail.svelte`; modify `App.svelte`.
...
**Interfaces:**
- Consumes `review.get` and `review.open`; `ReviewEntry` as declared in `review-store.ts`.
...
4. A failed review shows its reason here in full.
5. `Open as file` keeps the existing path to the review as markdown in an editor — a long review still needs search, folding and copy.
6. A running review shows progress as it streams rather than a static spinner, if the existing runner makes that available; if it does not, say so in the report rather than faking it.
...
**Deliverable:** all five gestures start a review that appears in the detail panel.
**Depends on:** phase 1.
**Ships independently:** yes.

## Task 1: Retarget the three existing gestures

**Files:** modify `App.svelte`.

**Read first:** the commit context menu's `Review this commit` and `Review with selected` entries, how `selectedHashes` supports shift-range selection, and the pull request detail's `Review with AI` handoff — all three already exist and already build a target.

**Requirements:**

1. Each of the three starts a review and shows it in the detail panel's review mode, rather than routing to the old panel.
2. The old panel continues to work in this phase. Both destinations may exist at once; phase 5 removes the old one.
3. **The provider and model come from settings, not from a control on the surface.** `gitGraphPro.aiReview.defaultProvider` already exists; the gestures read the configured provider rather than asking per run. This is what lets a review start from a context menu at all — there is nowhere to put two dropdowns in one.
...
| 2 | A review runs with the provider from settings, with no per-run provider control anywhere in the flow |
...
**Read first:** the branch context menu, which already carries `Compare with '<current>'` — the new branch review item is its sibling and should read like one. And the working-tree row, and whatever `diffWorkingTree` already provides.
...
| Spec clause | Task |
|---|---|
| Three existing gestures retargeted | 1 |
| Provider and model become settings, not per-run controls | 1 |
| No-provider case explained at every entry point | 1 |
...
5. **Every `review.*` method must still have a caller.** A method left with none is dead code that will be maintained by someone who does not know it is dead; a method whose only caller was deleted is a feature silently removed. Enumerate them and say which is which in the report.
...
| 2 | Every `review.*` method has a caller, or is removed with its removal stated |
...
| **2** | All five gestures start a review | 1 | Yes | Each gesture produces a review in the detail panel; with no provider, each explains what is missing rather than offering a dead control |
...
| **5** | The old panel removed | 2, 4 | Yes | One webview host; every `review.*` method has a caller; reviews stored before this plan still load |
